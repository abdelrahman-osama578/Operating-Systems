...
public class OperatingSystem {
    ...
    public OperatingSystem(OSDashboard gui, String schedulerType) {
        this.gui = gui;
    ...
    public void runSchedulerRR() {
        ...
        while (isRunning && hasUnfinishedProcesses()) {
            ...
            if (readyQueue.isEmpty()) {
                recordIdleTime();
                clock++; printSystemState(null); pauseExecution(); continue;
            }
    ...
    public void runSchedulerHRRN() {
        ...
        while (isRunning && hasUnfinishedProcesses()) {
            ...
            // STRICT FORMULA: Ratio = (Wait + Burst) / Burst
            for (PCB p : readyQueue) {
                int waitingTime = (clock - p.arrivalTime) - p.cpuCycles; // True waiting time
    ...
    public void runSchedulerMLFQ() {
        ...
        while (isRunning && hasUnfinishedProcesses()) {
            ...
            while (isRunning && currentProcess.quantumUsed < currentQuantum && currentProcess.state == State.RUNNING) {
                ...
                // SLIDE FIX: Strict Cross-Queue Preemption
                ...
                printSystemState(currentProcess); pauseExecution();
                if (preemptedByHigher) break; // Drop everything and yield CPU
            }
            ...
            if (isRunning && currentProcess.state == State.RUNNING) {
                currentProcess.state = State.READY;
                // Only demote if they exhausted their full time slice without blocking/preempting
                ...
                currentProcess.quantumUsed = 0; // Reset quantum since it voluntarily yielded
            }
        }
        finishOS();
    }
    ...
    private void recordGantt(int pid, int start, int end) {
        if (end > start) {
            if (!timeline.isEmpty() && timeline.get(timeline.size()-1).pid == pid && timeline.get(timeline.size()-1).end == start) {
                ...
                timeline.add(new GanttRecord(pid, start, end));
            }
        }
    }
    ...
    private void recordIdleTime() {
        if (!timeline.isEmpty() && timeline.get(timeline.size()-1).pid == 0 && timeline.get(timeline.size()-1).end == clock) {
            ...
            timeline.add(new GanttRecord(0, clock, clock + 1)); 
        }
    }
    ...
    private void finishOS() {
        if (!isRunning) return;
        if(gui != null) {
            gui.updateDashboard(clock, "SYSTEM HALTED", getMemoryArray(), "\n>>> ALL PROCESSES FINISHED. SYSTEM HALTED.");
    ...
    // Changed to return a boolean so the scheduler knows if it hit EOF without counting a cycle
        ...
        switch (command) {
            ...
            case "assign":
                if (tokens.length == 3 && tokens[2].equals("input")) {
                    ...
                    try {
                        javax.swing.SwingUtilities.invokeAndWait(() -> { result[0] = gui.requestInput(pcb.processID, tokens[1]); });
                    ...
                    setVariable(pcb, tokens[1], getVariable(pcb, tokens[2]));
                }
                break;
            case "printFromTo":
                int start = Integer.parseInt(getVariable(pcb, tokens[1]));
                int end = Integer.parseInt(getVariable(pcb, tokens[2]));
    ...
    private void setVariable(PCB pcb, String varName, String value) {
        for (int i = pcb.maxMemoryBoundary - 2; i <= pcb.maxMemoryBoundary; i++) {
            ...
            if (currentMem == null || currentMem.startsWith("VAR_" + varName + "=") || currentMem.equals("EMPTY_VAR")) {
                memory.write(i, "VAR_" + varName + "=" + value);
                return;
            }
        }
    }
    ...
    private String actualReadFile(String filename, int processId) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            if(gui != null) gui.appendLog("\n>> [PROCESS " + processId + "] DISK ERROR: " + filename + " not found!");
    ...
    private void loadProcessToMemory(String filename, int pid) {
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            if (minBoundary != -1) {
                ...
                for (int i = maxBoundary - 2; i <= maxBoundary; i++) memory.write(i, "EMPTY_VAR");
    ...
    private String[] getMemoryArray() {
        ...
        for(int i = 0; i < 40; i++) memArray[i] = memory.read(i);
        return memArray;
    }

    private void printSystemState(PCB current) {
        StringBuilder log = new StringBuilder();
        log.append("--- State at Clock ").append(clock).append(" ---\n");
        ...
        if (activeScheduler.equals("MLFQ")) {
            ...
            log.append("Ready Queue: ");
            for(PCB p : readyQueue) log.append("P").append(p.processID).append(" ");
        }
        
        log.append("\nBlocked (Input): ").append(userInput.blockedQueue.size());
        log.append("\nBlocked (Output): ").append(userOutput.blockedQueue.size());
        log.append("\nBlocked (File): ").append(fileAccess.blockedQueue.size());
        log.append("\n----------------------");
        
        if (gui != null) {
            String currentStr = current != null ? "P" + current.processID : "None";
            gui.updateDashboard(clock, currentStr, getMemoryArray(), log.toString());
        }
    }
}