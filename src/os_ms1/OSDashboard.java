...
public class OSDashboard extends JFrame {
    ...
    public void bootOS() {
        ...
        os = new OperatingSystem(this, selectedMode);
    ...
    public void showMetricsReport(String scheduler, List<OperatingSystem.GanttRecord> timeline, List<PCB> processes) {
        ...
        // 1. The Colorful Custom Gantt Chart
        ...
        JPanel ganttPanel = new JPanel() {
            ...
            protected void paintComponent(Graphics g) {
                ...
                for(int i=0; i<=totalTime; i++) {
                    ...
                    if (i % 5 == 0 || i == totalTime) { // Draw numbers every 5 ticks
                        g2d.setColor(TEXT_LIGHT);
                        g2d.drawString(String.valueOf(i), x - 5, startY + 75);
                        g2d.setColor(BORDER_COLOR);
                    }
                }

                // Draw Colored Process Blocks
                for (OperatingSystem.GanttRecord r : timeline) {
                    ...
                    if (r.pid == 0) g2d.setColor(new Color(50, 50, 50)); // IDLE (Grey)
                    else if (r.pid == 1) g2d.setColor(new Color(231, 76, 60)); // P1 (Red)
                    else if (r.pid == 2) g2d.setColor(new Color(52, 152, 219)); // P2 (Blue)
                    else if (r.pid == 3) g2d.setColor(new Color(46, 204, 113)); // P3 (Green)
                    else g2d.setColor(ACCENT_BLUE);

                    g2d.fillRect(rectX, startY, rectW, 40);
                    g2d.setColor(Color.WHITE);
                    g2d.drawRect(rectX, startY, rectW, 40);
        ...
        double tResp = 0, tTurn = 0, tWait = 0, tUtil = 0;
        int count = 0;
        ...
        for (PCB p : processes) {
            ...
            int waiting = turnaround - actualBurst;
            double util = turnaround == 0 ? 0 : actualBurst / (double) turnaround;
        ...
        if (count > 0) {
            model.addRow(new Object[]{ "AVG", "-", "-", "-", "-", String.format("%.2f", tResp/count), 
        ...
        dialog.add(chartContainer, BorderLayout.NORTH);
    ...
    private class ArtisticBackgroundPanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            g2d.setColor(new Color(29, 155, 240, 15)); 
            Path2D.Double poly1 = new Path2D.Double();
            poly1.moveTo(0, getHeight() * 0.8); poly1.lineTo(getWidth() * 0.6, 0); poly1.lineTo(0, 0); poly1.closePath();
            g2d.fill(poly1);
            ...
            g2d.setPaint(gp); g2d.fillOval(getWidth() - 400, getHeight() - 400, 600, 600);
    ...
    public String requestInput(int processId, String varName) {
        ...
        inputField.setBackground(BG_DARK); inputField.setForeground(ACCENT_BLUE);
        ...
        JButton submitBtn = createStyledButton("SUBMIT DATA");
        final String[] result = new String[1]; 
        submitBtn.addActionListener(e -> { result[0] = inputField.getText(); dialog.dispose(); });
        inputField.addActionListener(e -> submitBtn.doClick()); 

        JPanel centerPanel = new JPanel(new GridLayout(2, 1, 10, 10));
        centerPanel.setOpaque(false); centerPanel.add(prompt); centerPanel.add(inputField);
    ...
    private JPanel createTopPanel() {
        ...
        schedulerDropdown.setBackground(PANEL_DARK);
        schedulerDropdown.setForeground(TEXT_LIGHT);
        
        JPanel leftPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        leftPanel.setOpaque(false);
        leftPanel.add(new JLabel("<html><font color='white'>Active Scheduler: </font></html>"));
        leftPanel.add(schedulerDropdown);

        JPanel statsPanel = new JPanel(new GridLayout(1, 2));
        statsPanel.setOpaque(false);
        clockLabel = new JLabel("System Clock: 0");
        styleLabel(clockLabel, 26, true); 
        currentProcessLabel = new JLabel("Running: None");
        styleLabel(currentProcessLabel, 22, false); 
        currentProcessLabel.setHorizontalAlignment(SwingConstants.RIGHT);
        statsPanel.add(clockLabel);
    ...
    private JPanel createMemoryPanel() {
        ...
        String[] columnNames = {"Address", "Data Content"};
        memoryTableModel = new DefaultTableModel(columnNames, 40);
        memoryTable = new JTable(memoryTableModel);
        
        memoryTable.setBackground(BG_DARK); memoryTable.setForeground(TEXT_LIGHT);
        memoryTable.setGridColor(BORDER_COLOR); memoryTable.getTableHeader().setBackground(PANEL_DARK);
        memoryTable.getTableHeader().setForeground(ACCENT_BLUE);
        ...
        scrollPane.getViewport().setBackground(BG_DARK);
        scrollPane.setBorder(null); // Border handled by titled border
    ...
    private JPanel createSidePanel() {
        ...
        logArea.setBackground(BG_DARK); logArea.setForeground(new Color(200, 220, 240));
        logArea.setFont(new Font("Monospaced", Font.PLAIN, 16)); logArea.setEditable(false);
    ...
    private JPanel createBottomPanel() {
        ...
        credits.setForeground(new Color(150, 160, 170));
        creditsPanel.add(credits);

        JPanel controlsPanel = new JPanel(new FlowLayout(FlowLayout.RIGHT, 15, 10));
        controlsPanel.setOpaque(false);

        JButton rebootButton = createStyledButton("REBOOT OS");
        rebootButton.setBackground(new Color(243, 156, 18)); 
        rebootButton.addActionListener(e -> bootOS());

        JButton stepButton = createStyledButton("EXECUTE CYCLE");
        stepButton.addActionListener(e -> { if (os != null) os.stepCycle(); });

        autoRunButton = createStyledButton("AUTO-RUN");
        autoRunButton.setBackground(new Color(46, 204, 113)); 
        autoRunButton.addActionListener(e -> {
            if (os != null) {
                os.toggleAutoRun();
                if (os.isAutoRun) {
                    autoRunButton.setBackground(new Color(231, 76, 60)); 
                    autoRunButton.setText("STOP AUTO");
                } else {
                    autoRunButton.setBackground(new Color(46, 204, 113)); 
                    autoRunButton.setText("AUTO-RUN");
                }
        ...
        controlsPanel.add(rebootButton);
        controlsPanel.add(stepButton);
        controlsPanel.add(autoRunButton);
        
        wrapper.add(creditsPanel, BorderLayout.WEST);
        wrapper.add(controlsPanel, BorderLayout.EAST);
        wrapper.setBorder(BorderFactory.createMatteBorder(1, 0, 0, 0, BORDER_COLOR));

        return wrapper;
    }

    private void styleLabel(JLabel label, int fontSize, boolean bold) {
        label.setForeground(TEXT_LIGHT); label.setFont(new Font("SansSerif", bold ? Font.BOLD : Font.PLAIN, fontSize));
    }
    ...
    private TitledBorder createStyledTitledBorder(String title) {
        ...
        border.setTitleColor(ACCENT_BLUE); border.setTitleFont(new Font("SansSerif", Font.BOLD, 16)); 
        return border;
    }

    private JButton createStyledButton(String text) {
        JButton button = new JButton(text);
        button.setBackground(ACCENT_BLUE); button.setForeground(Color.WHITE); button.setFocusPainted(false);
    ...
    public void updateDashboard(int currentClock, String currentProcess, String[] memoryState, String logs) {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            OSDashboard gui = new OSDashboard();
            gui.setVisible(true);
            gui.bootOS(); 
        });
    }
